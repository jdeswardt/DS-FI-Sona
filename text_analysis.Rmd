...
sona_words %>% group_by(FILE) %>% count() %>% head(5) %>% arrange(desc(n))
```
...
word_sentiments <- sona_words %>% left_join(get_sentiments("bing")) %>%
...
```{r}
#Sentiments over entire speeches
speech_sentiments <- word_sentiments %>%
                     group_by(YEAR) %>%
                     summarize(NET_SENTIMENT=(sum(SENTIMENT=="positive") - sum(SENTIMENT=="negative")))
plot(x=speech_sentiments$YEAR, y=speech_sentiments$NET_SENTIMENT, type="l", ylab="SENTIMENT", xlab="YEAR", main="AVERGAE SPEECH SENTIMENT OVER TIME")
```
...
sona_gamma %>% group_by(FILE) %>% summarize(ntopic1 = sum(topic_1 > 0.5))
```
